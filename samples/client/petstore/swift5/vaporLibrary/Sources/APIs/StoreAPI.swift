...
open class StoreAPI {
    ...
    open class func getInventory(headers: HTTPHeaders = PetstoreClient.customHeaders, beforeSend: (inout ClientRequest) throws -> () = { _ in }) -> EventLoopFuture<GetInventory> {
        ...
        return apiClient.send(.GET, headers: headers, to: URI(string: URLString)) { request in
            ...
            switch response.status.code {
            case 200:
                return .http200(value: try? response.content.decode([String: Int].self), raw: response)
            default:
                return .http0(value: try? response.content.decode([String: Int].self), raw: response)
            }
        }
    }
    ...
    open class func getOrderById(orderId: Int64, headers: HTTPHeaders = PetstoreClient.customHeaders, beforeSend: (inout ClientRequest) throws -> () = { _ in }) -> EventLoopFuture<GetOrderById> {
        ...
        return apiClient.send(.GET, headers: headers, to: URI(string: URLString)) { request in
            ...
            switch response.status.code {
            case 200:
                return .http200(value: try? response.content.decode(Order.self), raw: response)
            case 400:
                return .http400(value: (), raw: response)
            case 404:
                return .http404(value: (), raw: response)
            default:
                return .http0(value: try? response.content.decode(Order.self), raw: response)
            }
        }
    }
